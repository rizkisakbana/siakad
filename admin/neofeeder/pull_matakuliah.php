...
$page_title = "Pull Mata Kuliah NeoFeeder";
$page_subtitle = "Menarik data mata kuliah dari NeoFeeder/PDDikti ke SIAKAD";

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pull_matakuliah'])) {
    $limit = max(1, min(500, (int) ($_POST['limit'] ?? 100)));
    $offset = max(0, (int) ($_POST['offset'] ?? 0));
    $ambil_detail = isset($_POST['ambil_detail']);

    $summary = pull_matakuliah_akademik($conn, $limit, $offset, $ambil_detail);
    $_SESSION['pull_matakuliah_summary'] = $summary;

    simpan_log($conn, $_SESSION['id_user'], "Pull mata kuliah NeoFeeder. Insert: {$summary['insert']}, Update: {$summary['update']}, Gagal: {$summary['gagal']}", "Neo Feeder");
    set_alert($summary['gagal'] > 0 ? 'warning' : 'success', 'Pull mata kuliah selesai diproses.');
    header("Location: pull_matakuliah.php");
    exit;
}
...
<main class="lg:ml-[270px] p-4 sm:p-6 lg:p-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Pull Mata Kuliah NeoFeeder</h2>
            <p class="text-sm text-slate-500">Mengambil GetListMataKuliah dan opsional GetDetailMataKuliah.</p>
        </div>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    <label class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-3 w-full text-sm text-slate-700">
                        ...
                        Lengkapi data dengan GetDetailMataKuliah
                    </label>
                </div>
                <div class="md:col-span-2">
                    <button type="submit" name="pull_matakuliah" value="1" onclick="return confirm('Tarik mata kuliah dari NeoFeeder sekarang?')" class="inline-flex items-center justify-center px-5 py-3 rounded-xl bg-blue-700 hover:bg-blue-800 text-white font-semibold">
                        <i class="fa-solid fa-cloud-arrow-down mr-2"></i> Pull Mata Kuliah
                    </button>
                </div>
            </form>
        </div>
        <aside class="bg-white rounded-2xl shadow-lg border border-slate-100 p-5 sm:p-6">
            ...
            <div class="space-y-4 text-sm">
                <div class="p-4 rounded-xl bg-blue-50 border border-blue-100"><p class="text-blue-700 font-semibold">Total Mata Kuliah</p><p class="text-2xl font-bold"><?= number_format($total_lokal); ?></p></div>
                ...
                <div class="p-4 rounded-xl bg-orange-50 border border-orange-100"><p class="text-orange-700 font-semibold">Kurikulum Lokal</p><p class="text-2xl font-bold"><?= number_format($total_kurikulum); ?></p><p class="mt-1">Wajib tersedia sebelum pull mata kuliah.</p></div>
            </div>
        </aside>
    </section>
</main>
