...
$total_lokal = 0;
$q_total = mysqli_query($conn, "SELECT COUNT(*) total FROM transkrip_mahasiswa");
if ($q_total) {
    $total_lokal = mysqli_fetch_assoc($q_total)['total'] ?? 0;
}

$total_mahasiswa_reg = 0;
$q_reg = mysqli_query($conn, "SELECT COUNT(*) total FROM mahasiswa WHERE id_registrasi_feeder IS NOT NULL AND id_registrasi_feeder <> ''");
if ($q_reg) {
    $total_mahasiswa_reg = mysqli_fetch_assoc($q_reg)['total'] ?? 0;
}
