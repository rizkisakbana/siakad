...
$page_title = "Pull Peserta Kelas / KRS NeoFeeder";
$page_subtitle = "Menarik peserta kelas sekaligus membentuk KRS lokal";
$summary_key = 'pull_peserta_kelas_summary';

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pull_data'])) {
    $limit = max(1, min(1000, (int) ($_POST['limit'] ?? 200)));
    $offset = max(0, (int) ($_POST['offset'] ?? 0));

    $summary = pull_peserta_krs_inti($conn, $limit, $offset);
    $_SESSION[$summary_key] = $summary;

    simpan_log($conn, $_SESSION['id_user'], "Pull peserta kelas/KRS NeoFeeder. Insert: {$summary['insert']}, Update: {$summary['update']}, Gagal: {$summary['gagal']}", "Neo Feeder");
    set_alert($summary['gagal'] > 0 ? 'warning' : 'success', 'Pull peserta kelas/KRS selesai diproses.');
    header("Location: pull_peserta_kelas.php");
    exit;
}
...
$total_peserta = nf_count($conn, "SELECT COUNT(*) total FROM peserta_kelas_kuliah");
$total_krs = nf_count($conn, "SELECT COUNT(*) total FROM krs");
$total_detail = nf_count($conn, "SELECT COUNT(*) total FROM krs_detail");
...
<main class="lg:ml-[270px] p-4 sm:p-6 lg:p-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Pull Peserta Kelas / KRS NeoFeeder</h2>
            <p class="text-sm text-slate-500">Mengambil GetPesertaKelasKuliah dan membentuk KRS/KRS detail lokal.</p>
        </div>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-slate-100 p-5 sm:p-6">
            <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2"><button type="submit" name="pull_data" value="1" onclick="return confirm('Tarik peserta kelas/KRS dari NeoFeeder sekarang?')" class="inline-flex items-center justify-center px-5 py-3 rounded-xl bg-blue-700 hover:bg-blue-800 text-white font-semibold"><i class="fa-solid fa-cloud-arrow-down mr-2"></i> Pull Peserta / KRS</button></div>
            </form>
        </div>
        <aside class="bg-white rounded-2xl shadow-lg border border-slate-100 p-5 sm:p-6">
            ...
            <div class="space-y-4 text-sm">
                <div class="p-4 rounded-xl bg-blue-50 border border-blue-100"><p class="text-blue-700 font-semibold">Peserta Kelas</p><p class="text-2xl font-bold"><?= number_format($total_peserta); ?></p></div>
                <div class="p-4 rounded-xl bg-green-50 border border-green-100"><p class="text-green-700 font-semibold">KRS</p><p class="text-2xl font-bold"><?= number_format($total_krs); ?></p></div>
                <div class="p-4 rounded-xl bg-purple-50 border border-purple-100"><p class="text-purple-700 font-semibold">KRS Detail</p><p class="text-2xl font-bold"><?= number_format($total_detail); ?></p></div>
                <div class="p-4 rounded-xl bg-orange-50 border border-orange-100"><p class="text-orange-700 font-semibold">Syarat</p><p>Pull kelas kuliah dan mahasiswa harus sudah dilakukan.</p></div>
            </div>
        </aside>
    </section>
</main>
