...
$page_title = "Data WhatsApp Gateway";
$page_subtitle = "Monitoring riwayat pengiriman WhatsApp sistem";
...
if (!empty($keyword)) {
    $where = "WHERE 
        ...
        whatsapp_log.tujuan_nomor LIKE '%$keyword%' OR
        whatsapp_log.isi_pesan LIKE '%$keyword%' OR
        whatsapp_log.status LIKE '%$keyword%'
    ";
}
...
$total_terkirim = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT COUNT(*) AS total FROM whatsapp_log WHERE status = 'terkirim'
"))['total'] ?? 0;

$total_pending = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT COUNT(*) AS total FROM whatsapp_log WHERE status = 'pending'
"))['total'] ?? 0;
...
$data_whatsapp = mysqli_query($conn, "
    SELECT 
        whatsapp_log.*,
        users.nama_lengkap,
        users.username
    ...
    LEFT JOIN users ON whatsapp_log.id_user = users.id_user
    $where
    ORDER BY whatsapp_log.id_whatsapp_log DESC
...
<main class="lg:ml-[270px] p-4 sm:p-6 lg:p-8">
    ...
    <section class="bg-white rounded-2xl shadow-lg border border-slate-100 p-4 sm:p-6 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <a href="data_whatsapp.php"
               class="p-5 rounded-2xl border border-green-200 bg-green-50 hover:bg-green-100 transition">
                <div class="w-11 h-11 rounded-xl bg-green-700 text-white flex items-center justify-center mb-4">
    ...
    <section class="bg-white rounded-2xl shadow-lg border border-slate-100 p-4 sm:p-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Riwayat WhatsApp Gateway</h2>
                <p class="text-sm text-slate-500 mt-1">
                    Menampilkan daftar WhatsApp yang diproses oleh sistem.
                </p>
            </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-slate-100 text-slate-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left">Nomor Tujuan</th>
                        <th class="px-4 py-3 text-left hidden lg:table-cell">Pengguna</th>
                        <th class="px-4 py-3 text-left hidden lg:table-cell">Pesan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php if (mysqli_num_rows($data_whatsapp) > 0): ?>
                        <?php $no = $offset + 1; ?>
                        <?php while ($row = mysqli_fetch_assoc($data_whatsapp)): ?>
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 hidden lg:table-cell">
                                    <div class="text-slate-700 max-w-md truncate">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="detail_whatsapp.php?id=<?= $row['id_whatsapp_log']; ?>"
                                        ...
                                        <a href="hapus_whatsapp.php?id=<?= $row['id_whatsapp_log']; ?>"
